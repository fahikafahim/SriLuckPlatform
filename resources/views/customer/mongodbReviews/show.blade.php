    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-8">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center mb-2">
                            ...
                            <span class="ml-2 text-gray-600">Posted on {{ $review->created_at->format('M d, Y') }}</span>
                        </div>
                        {{-- Review text --}}
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                @foreach ($review->images as $image)
                                    <div class="border rounded-md overflow-hidden">
                                        <img src="{{ Storage::url($image) }}" alt="Review image" class="w-full h-48 object-cover">
                                    </div>
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="mt-12 bg-white shadow-sm sm:rounded-lg p-8">
                ...
                {{-- Fetch other reviews excluding current --}}
                @php
                                ...
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <div class="flex items-center mb-1">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <span class="text-lg {{ $i <= $other->rating ? 'text-yellow-400' : 'text-gray-300' }}">★</span>
                                            @endfor
                                        </div>
                                        {{-- Comment Text --}}
                                        <p class="text-gray-700">{{ $other->comment }}</p>
                                        <small class="text-gray-500">Order #{{ $other->order_id }} • {{ $other->created_at->format('M d, Y') }}</small>
                                    </div>
