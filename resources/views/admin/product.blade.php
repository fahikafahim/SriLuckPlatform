    ...
    <div class="flex h-screen bg-gray-100">
        ...
        <div class="flex-1 p-6 bg-gray-50 overflow-auto">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <button id="showFormBtn" class="flex items-center bg-navy-800 hover:bg-navy-900 text-white font-medium py-2 px-4 rounded-lg transition-all duration-300 hover:shadow-md">
                    <i class="fas fa-plus-circle mr-2"></i>Add New Product
                </button>
            </div>

            <!-- Product Creation / Editing Form Popup (Initially Hidden) -->
            <div id="productFormOverlay" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center hidden">
                <div id="productFormContainer" class="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all duration-300">
                    <div class="p-6">
                        ...
                        <form id="productForm" enctype="multipart/form-data">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" id="name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500 transition-all">
                                </div>
                                <div>
                                    <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Price</label>
                                    <input type="number" step="0.01" id="price" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500 transition-all">
                                </div>
                                <div class="md:col-span-2">
                                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                                    <textarea id="description" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500 transition-all"></textarea>
                                </div>
                                <div>
                                    <label for="color" class="block text-sm font-medium text-gray-700 mb-1">Color</label>
                                    <input type="text" id="color" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500 transition-all">
                                </div>
                                <div>
                                    <label for="size" class="block text-sm font-medium text-gray-700 mb-1">Size</label>
                                    <input type="text" id="size" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy-500 focus:border-navy-500 transition-all">
                                </div>
                                <div class="md:col-span-2">
                                    <label for="edit-image" class="block text-sm font-medium text-gray-700 mb-1">Product Image</label>
                                    <div class="flex items-center space-x-4">
                                        <div class="relative flex-1">
                                            <input type="file" id="edit-image" name="image" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer">
                                            <div class="px-4 py-2 border border-gray-300 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                                                <span class="text-sm text-gray-700">Choose file</span>
                                            </div>
                                        </div>
                                        <span id="file-name" class="text-sm text-gray-500 truncate flex-1">No file chosen</span>
                                    </div>
                                    ...
                                    <div id="current-image-container" class="mt-4">
                                        ...
                                        <div class="w-32 h-32 border border-gray-200 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
                                            <img id="current-image-preview" src="#" alt="Preview" class="w-full h-full object-cover hidden">
                            ...
                            <div class="mt-6 flex justify-end space-x-3">
                                <button type="button" id="cancelFormBtn" class="px-5 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
                                    Cancel
                                </button>
                                <button type="submit" class="px-5 py-2 text-sm font-medium text-white bg-navy-600 hover:bg-navy-700 rounded-lg transition-colors focus:ring-2 focus:ring-navy-500 focus:ring-offset-2">
                                    Save Product
                                </button>
                            </div>
                        </form>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-navy-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Size</th>
...
<style>
    ...
    .bg-navy-300 { background-color: #9fb3c8; }
    .bg-navy-400 { background-color: #829ab1; }
    .bg-navy-500 { background-color: #627d98; }
...
<!-- Script remains the same as in your original code -->
<script>
/*
 * Constants and Element References
 */
const API_URL = '/api/products'; // API endpoint for product CRUD operations
const token = "2|QzXDnxEPWAQzCeKqad8nUmBmqjir47yaSOEfhWxO6f53b38f"; // Authentication token for API access
 ...
 * If no products found, display a message instead
 */
    ...
    products.forEach(product => {
        ...
        row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap">
                ${product.image_url ? `<img src="${product.image_url}" alt="${product.name}" class="w-16 h-16 object-cover rounded-md">` : 'No image'}
            </td>
